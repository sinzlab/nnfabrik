...
def build_core(input_channels, hidden_channels=12, rec_channels=36, input_kern=13, hidden_kern=3, rec_kern=1,
               layers=3, gamma_hidden=0.1, gamma_input=20.0, gamma_rec=0.1, momentum=0.1, bias=True, skip=2, pad_input=1):
    return cores.StackedFeatureGRUCore(input_channels, hidden_channels=hidden_channels, rec_channels=rec_channels,
                                       input_kern=input_kern, hidden_kern=hidden_kern, rec_kern=rec_kern, layers=layers,
                                       gamma_hidden=gamma_hidden, gamma_input=gamma_input, gamma_rec=gamma_rec,
                                       momentum=momentum, bias=bias, skip=skip, pad_input=pad_input)

def build_readout(in_shape, neurons, gamma_features=1.0, positive=False, pool_steps=2, kernel_size=4, stride=4):
    return readouts.STPool3dReadout(in_shape=in_shape, neurons=neurons, gamma_features=gamma_features, positive=positive,
                             pool_steps=pool_steps, kernel_size=kernel_size, stride=stride)

def build_modulator(data_keys, input_channels, hidden_channels=50, bias=True, gamma_modulator=0, offset=1):
    return modulators.GateGRUModulator(data_keys, input_channels, hidden_channels=hidden_channels,
                              bias=bias, gamma_modulator=gamma_modulator, offset=offset)
...
def build_network(dataloaders, burn_in=15, core_config=None, readout_config=None, shifter_config=None, modulator_config=None):
    ...
    model = base.CorePlusReadout3d(core, readout, nonlinearity=Elu1(), shifter=shifter,

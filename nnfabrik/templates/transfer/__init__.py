...
class TransferredTrainedModelBase(TrainedModelBase):
    ...
    def get_full_config(self, key=None, include_state_dict=True, include_trainer=True):
        ...
        if key["transfer_step"] > 0:
            ...
            if self.DataStorage & prev_key:
                with tempfile.TemporaryDirectory() as temp_dir:
                    train_path = (self.DataStorage & prev_key).fetch1(
                        "transfer_train_data", download_path=temp_dir
                    )
                    test_path = (self.DataStorage & prev_key).fetch1(
                        "transfer_test_data", download_path=temp_dir
                    )
                    ret["dataset_config"]["transfer_train_data"] = np.load(train_path)
                    ret["dataset_config"]["transfer_test_data"] = np.load(test_path)
        return ret
    ...
    def make(self, key):
        ...
        if key["data_transfer"]:
            # generate data
            train, test, model_state = trainer(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if key["data_transfer"]:
                train_path = os.path.join(temp_dir, filename + "_train.npy")
                test_path = os.path.join(temp_dir, filename + "_test.npy")

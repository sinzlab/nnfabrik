...
def resolve_fn(fn_name, default_base):
    ...
    if not callable(fn_obj) and not hasattr(fn_obj, "__call__"):
...
def get_model(
    ...
    if issubclass(model_fn, ModelBuilder):
        model_fn = model_fn()  # instantiate the class
...
def get_data(dataset_fn, dataset_config):
    ...
    if issubclass(dataset_fn, Dataset):
        dataset_fn = dataset_fn()  # instantiate the class

    return dataset_fn(**dataset_config)


def get_trainer(trainer_fn, data_loaders, model, seed, trainer_config=None):
    """
    ...
    if issubclass(trainer_fn, Trainer):
        trainer_fn = trainer_fn(model, data_loaders, seed, **trainer_config)
    elif trainer_config is not None:
        trainer_fn = partial(trainer_fn, model, data_loaders, seed, **trainer_config)
...
def get_all_parts(
    ...
    data_loaders = get_data(dataset_fn, dataset_config)
    ...
    if trainer_fn is not None:
        trainer = get_trainer(trainer_fn, data_loaders, model, seed, trainer_config)
        return data_loaders, model, trainer
    else:
